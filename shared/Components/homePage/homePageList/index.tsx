...
export default function FormListing({ data, refetch }: FormListing) {
  ...
  return (
    <tr className='bg-white border-b  hover:bg-gray-50'>
      ...
      <td className='px-6 py-[8px] flex gap-2'>
        <Link href={`/formbuilder/${data?.sFormId}`} className=''>
          <div className='hover:bg-primary-300 rounded-[50%] h-fit w-fit p-2'>
          <CustomImage src={EditIcon} height={18} width={18} />
          </div>
        </Link>
        <button className=''>
          <div onClick={() =>  handleDelete(data?.sFormId)} className='hover:bg-red-400 rounded-[50%] h-fit w-fit p-2'>
          <CustomImage src={DeleteIcon} height={18} width={18} />
          </div>
        </button>
      </td>
    </tr>
