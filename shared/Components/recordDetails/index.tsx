...
export default function ReportDetail({ data, deleteMutation }: any) {
  const t = useI18n()
      ...
      {data && (
        <>
          ...
          <div className='text-lg p-4'>
            <span className='text-secondary-500 dark:text-secondary-400 mx-1'>Submission Date :</span> {formatDate(data?.dCreatedAt)}
          </div>
              ...
              {data?.aAnswers?.map((ans: any, i: number) => {
                return (
                  <div className='flex w-full justify-between md:text-lg text-sm mb-2' key={i}>
                    <div className='text-secondary-500 dark:text-secondary-400'>{ans?.sLabel}</div>
